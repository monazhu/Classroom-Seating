...
setwd("C:/Users/monaj/Dropbox/CANB Lab/Classroom (new)")

# raw data file with both chosen and simulated random seats
comb<-read.xlsx("Complete Classroom Data_FULLDATA_21Mar2017.xlsx", sheetName="Sheet1")
...
comb$MovedFromPrev<-ifelse(comb$DistFromPrev<.1, 1, 0) # did individuals move (y/n)
...
comb.c$Time2<-(comb.c$Time)^2
comb.cen<-transform(comb.c, Time=scale(Time, scale=F), Order=scale(Order, scale=F))
comb.cen$Time2<-(comb.cen$Time)^2

once<-subset(comb.c, ClassFreq=="Once")
once.cen<-subset(comb.c, ClassFreq=="Once")
twice<-subset(comb.c, ClassFreq=="Twice")
twice.cen<-subset(comb.cen, ClassFreq=="Twice")

sub<-subset(comb.c, FinalExam=="No")
sub.cen<-subset(comb.cen, FinalExam=="No")
```
...
```{r}
linear<-lmer(DistFromPrev~Time+Order+(1|UserName), data = comb.c, REML=F, na.action="na.omit")
summary(linear)
...
```{r}
prop<-glmer(MovedFromPrev~Time+Order+(1|UserName), data=comb.c, family=binomial(link="logit"),na.action = na.omit) 

summary(prop)
confint(prop, method="Wald")
...
linear0<-lmer(DistFromPrev~Time+Order+(1|UserName), data = nonzero, REML=F, na.action="na.omit")
summary(linear0)
confint(linear0, method="Wald")
```

#### MAIN Curvilinear analysis (seems to be issues with this):

```{r}
curvilinear<-lmer(DistFromPrev~Time+Order+Time2+(1|UserName), data = comb.c, REML=F, na.action="na.omit")

summary(curvilinear)
confint(curvilinear, method="Wald")
```
...
```{r}
curvilinear2<-lmer(DistFromPrev~Time+Order+Time2+(1|UserName), data = sub, REML=F, na.action="na.omit")
summary(curvilinear2)
confint(curvilinear2, method="Wald")
...
plot1<-ggplot(comb.new, aes(x=Time, y=DistFromPrev)) +
  geom_point(aes(size=freq), alpha=0.2, show.legend = F, colour="grey")+
  geom_smooth(method=lm, formula = y ~ x + I(x^2), se=T, fullrange=TRUE, colour="gray55", alpha=.7)+
...
png(file = "Classroom_Figure1_Updated.png", bg = "transparent", type = c("cairo"), width=2400, height=2200, res=300)
...
```{r}
melted<-melt(lecex, id=c(1:9, 12:20))
...
ppl.mod<-lme(DistFromPrev ~ Time+Order+freq, random= ~1+Time|UserName, data = comb.ppl[comb.ppl$freq<18,], na.action = "na.omit", method="ML")
summary(ppl.mod)
```
